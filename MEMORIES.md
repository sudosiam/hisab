...
Writes must call `scheduleGeneralLedgerRefresh` (debounced) or `refreshGeneralLedgerAfterWrite` (await). Report/GL readers already call `awaitPendingLedgerRefresh` before querying.
