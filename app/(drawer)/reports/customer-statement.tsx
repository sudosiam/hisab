...
export default function CustomerStatementReportScreen() {
  ...
  const { error, retry } = useFocusRefresh(load, [customerName, fromDate, toDate, refreshKey]);
  ...
  const showStatement = customerName.trim().length > 0 && !hint && (!loading || lines.length > 0);
